 ...
 *
 * Allows to select a bundled environment -> if that's the case,
 *
 * @author Marcel Wiedenmann, KNIME GmbH, Konstanz, Germany
 * @author Carsten Haubold, KNIME GmbH, Konstanz, Germany
 */
@SuppressWarnings("serial") // Not intended for serialization.
public final class PythonFixedVersionOrBundledExecutableSelectionPanel extends PythonExecutableSelectionPanel {
     ...
     */
    public PythonFixedVersionOrBundledExecutableSelectionPanel(final NodeDialogPane dialog, final PythonCommandConfig config) {
        ...
        m_usePreferencesButton.addActionListener(e -> updateModel(null, false));
    ...
    private void updateModel(final String flowVariableName, final boolean useBundledEnv) {
        if (!useBundledEnv) {
            m_flowVariableModel.setInputVariableName(flowVariableName);
        } else {
            m_flowVariableModel.setInputVariableName(null);
            updateConfigAndView();
        }
    }

    /**
     * See {@link PythonFixedVersionOrBundledExecutableSelectionPanel#updateView()} for the documentation of the different
     * states.
     */
    private void updateConfigAndView() {
        ...
        if (m_useBundledEnvButton.isSelected()) {
            ...
            if (variableValue.isPresent()) {
                ...
                // State 4
                NodeLogger.getLogger(PythonFixedVersionOrBundledExecutableSelectionPanel.class).warn("The variable \"" +
     ...
     * </ul>
     *
     * NEW: if flowVariableName == BUNDLED_ENV_COMMAND_STRING -> use our bundled env!
     */
