...
class MyStandardKnimeNodeModel():
    ...
    def execute(self, inObjects, exec_context: knime.ExecutionContext):
        ...
        row_keys = pa.array(("Row" + str(i) for i in range(len(result))), type=pa.string(), size=len(result))
        result = pa.Table.from_arrays([row_keys, result], names=["Row ID", "Result"])
        # TODO: get file path from KNIME, set metadata to fixed version?
        out_table_file_path = "/tmp/my_test_file.knable"
        schema = result.schema
        schema = schema.remove_metadata()
        metadata = {"KNIME:basic:factoryVersions": "0,0", "KNIME:basic:chunkSize": str(len(result))}  # TODO: change to actual chunk size once we use chunking
        schema = schema.add_metadata(metadata)
        writer = RecordBatchFileWriter(out_table_file_path, schema)
